...
closes a G1 debt. Start from `notes/race-mechanics/goblin-phase-g2-partA-report.md` (investigation + numbers
model) and `notes/race-mechanics/goblin-phase-g2-partB-as-built.md` (what shipped) — **build-verified only,
...
| `RestedBehavior.cs` | `EntityBehavior` (`OnGameTick`, `OnEntityReceiveSaturation`), registered as `"rfrested"` (`RFMechanicsModSystem.cs:35`) | **None — race-agnostic as shipped, see below** | `EnableRested`, `RestedIdleGainPerSecond`, `RestedEatGainFlat`, `RestedStatWriteThreshold`, `RestedMiningSpeedBonus`, `RestedForageDropBonus`, `RestedWildCropDropBonus`, `RestedHungerRateBonus` | **Not previously in this table — added 2026-08-13, a gap this doc already flagged on itself below (Cross-mod context).** Server-authoritative Rested float (0–1) feeding `miningSpeedMul`/`forageDropRate`/`wildCropDropRate`/`hungerrate` via threshold-gated `Stats.Set` writes (source `"rested"`). Gain: idle tick + flat per-eat bonus. Drain: external, via the three patches below. **Applies to every player regardless of race — decided direction (Elf-only, tied to trees/nature) not yet implemented.** |
| `RestedBlockBreakPatch.cs` | `Block.OnBlockBroken` (postfix) | None (feeds race-agnostic `RestedBehavior`) | `EnableRested`, `RestedBlockBreakDrain` | **Not previously in this table — added 2026-08-13.** Drains Rested a flat amount per block broken, any material. |
| `RestedToolUsePatch.cs` | `CollectibleObject.OnHeldInteractStep` (postfix) | None | `EnableRested`, `RestedToolUseDrainPerSecond` | **Not previously in this table — added 2026-08-13.** Drains Rested for sustained tool use, computed as a per-second rate since the underlying method fires ~every 20ms. |
| `RestedDurabilityPatch.cs` | `CollectibleObject.DamageItem` (prefix) | None | `EnableRestedDurability` (default **false**), `RestedDurabilityBonus` | **Not previously in this table — added 2026-08-13.** Off by default. Scales tool durability loss by current Rested value when enabled. |
...
| `GoblinSpitChargeGrantPatch.cs` | `CollectibleObject.tryEatStop` (postfix) | `GoblinTraitCode` | `EnableGoblinSpitCharges`, `SpitChargesPerRot`, `SpitChargeCap` | **New 2026-08-12 (Phase G3) — not previously in this table.** Grants spit charges (capped) when a goblin finishes eating `game:rot`, gated on the same completion threshold (`secondsUsed >= 0.95f`) vanilla uses. Deployed; **not yet smoke-tested in-game.** |
| `RfGoblinSpitRepairBehavior.cs` | `BlockBehavior.OnBlockInteractStart`, registered as `"RfGoblinSpitRepair"` (`RFMechanicsModSystem.cs:43`, `RegisterBlockBehaviorClass`) | `GoblinTraitCode` | `EnableGoblinSpitCharges`, `SpitRepairGain` | **New 2026-08-12 (Phase G3) — not previously in this table.** Lets a goblin spend a spit charge via empty-hand interact to repair a reparable block, mirroring vanilla's own `BehaviorReparable` repair-application math. Deployed; **not yet smoke-tested in-game.** |
...
| `RFMechanicsModSystem.cs` | — | — | — | **Corrected 2026-08-13** — the registration list below was previously incomplete. `Start()`: load config, register entity/crop/block behavior classes `rfrested`, `rftreeproximity`, `rfthew`, `rfband`, `rfburn`, `rfgoblintunnel`, `rfgoblinrotaura`, `RfGoblinCropStunt`, `RfGoblinSpitRepair` (nine calls, `RFMechanicsModSystem.cs:35-43`), then `harmony.PatchAll(Assembly.GetExecutingAssembly())`. `StartServerSide()`: registers `/dwarfdepth`, `/rfdiag`, `/rfstatsfix`, `/rfphase0`, `/rfthew`, and (2026-08-12, previously undocumented here) `/rfrotdiag`/`/rfrotaura` — all server-side only, use the full `/name`, not the `.` shortcut. |
