...
| `GoblinDigModifierBehavior.cs` | `BlockBehavior.GetMiningSpeedModifier` (own extension point, **not** a Harmony patch) | `GoblinTraitCode` | `EnableGoblinDigBonus`, `GoblinBareHandDigRate` | **New 2026-08-06 (Phase G2).** Goblin bare-hand dig bonus on Soil/Sand/Gravel-tier blocks — returns `GoblinBareHandDigRate` (default 8.0, beats a steel shovel on every material) for goblins, `1.0` otherwise. Bare-hand-only by construction: a held tool's `OnBlockBreaking` overwrites `Block.OnGettingBroken`'s result entirely, so this behavior's effect is discarded whenever a tool is in hand. Attached via JSON patch (`patches/goblin-dig-blockbehavior.json`) to `soil`/`sand`/`gravel`/`packeddirt`/`aridpackeddirt`, and directly in the two new spit-packed blocktypes below. Registered via `RegisterBlockBehaviorClass`, not `PatchAll`. **Build-verified only.** |
...
| `RFGoblinTunnelBehavior.cs` | `EntityBehavior` (`OnGameTick`), attached via `patches/seraph-goblintunnel.json` | `GoblinTraitCode` (checked in `IsGoblin()`, inline, not a Harmony patch) | `EnableGoblinTunnelSpeed`, `GoblinTunnelSpeedBonus`, `GoblinTunnelStatWriteThreshold` | **New 2026-08-06 (Phase G2).** Walkspeed bonus for goblins tunneling under diggable earth — `RFTreeProximityBehavior`'s exact pattern (3s tick, not Thew/Band's 6s), but a narrow 2-block directional column check (`headY+1`/`headY+2`) instead of a radius `WalkBlocks` scan. Hysteresis: entry needs only the near sample, exit needs both to fail. Writes `Stats.Set("walkspeed", "tunneling", value)`. Condition (`BlockMaterial ∈ {Soil,Sand,Gravel} && RequiredMiningTier==0`) already covers `packeddirt`/spit-packed variants automatically. **Build-verified only.** |
| `GoblinSpitPackingPatch.cs` | `Block.OnBlockBroken` (postfix) | `GoblinTraitCode` | `EnableGoblinSpitPacking` | **New 2026-08-06 (Phase G2).** Converts all six face-adjacent Soil/Sand/Gravel neighbors of a goblin's break to their spit-packed variant (`survival:packeddirt` for Soil; the new `spitpackedsand-{rock}`/`spitpackedgravel-{rock}` blocktypes for Sand/Gravel), unconditionally rather than replicating `BlockBehaviorUnstableFalling`'s fall-decision logic. Relies on winning a race against `TryFalling`'s deferred `EnqueueMainThreadTask` closure by converting synchronously before it drains — reasoned from source, **not yet verified in-game**; the documented fallback (switch to a prefix) was not built speculatively. Ships ungated by primer/rot state (G3 scope). **Build-verified only, race-window behavior specifically unconfirmed.** |
| `ElfLeafDropPatch.cs` | `Block.GetDrops` (postfix, appends to `__result`) | `ElfTraitCode` | `EnableElfLeafGathering` | **New 2026-08-06 (Phase G2).** Appends a self-drop (`leaves-placed-{wood}`/`leavesbranchy-placed-{wood}`, grown-stage-to-placed conversion) to vanilla's existing `treeseed`/`stick` drops for elves breaking `leaves-`/`leavesbranchy-` blocks — appends, does not replace. Coexists with `OreYieldPatch` on the same `Block.GetDrops` hook (different material gate). **Closes G1's open branchy-leaves ingredient-sourcing gap** (leaves never dropped themselves before this — see `goblin-phase-g1-as-built.md`). **Build-verified only.** |
...
- **Goblin Phase G2 (2026-08-06): build-verified only (`dotnet build`, 0 errors), not yet
  deployed to the live install or smoke-tested in-game.** Six new mechanics across dig speed,
  climbing, tunnel speed, spit-packed earth conversion, and Elf leaf gathering — see
  `notes/goblin-phase-g2-partB-as-built.md` for the full smoke-test handoff checklist. The
  spit-packed conversion's race-beating mechanism (`GoblinSpitPackingPatch.cs`) is the item
  most likely to need a follow-up fix (documented prefix-based fallback if the postfix version
  loses the race against the falling-block spawn) — flagged, not built speculatively.
